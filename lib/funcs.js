...
async function addArt(chara) {
    ...
    await chara.artifacts.forEach(r => {
        artData[artList[r.artifactData.equipType]] = {
            "type": r.artifactData.set.name.get('jp'),
            "Level": r.level - 1,
            "rarelity": r.artifactData.stars,
            "main": {
                "option": !removeStatList.includes(r.mainstat.id) && r.mainstat.isPercent && r.mainstat.type.get('jp') != "元素チャージ効率" && r.mainstat.type.get('jp') != "会心ダメージ" && r.mainstat.type.get('jp') != "会心率" ? r.mainstat.type.get('jp') + "パーセンテージ" : r.mainstat.type.get('jp'),
        ...
        if (r.substats) {
            r.substats.total.forEach(s => {
                sub.push({
                    "option": s.isPercent && s.type.get('jp') != "元素チャージ効率" && s.type.get('jp') != "会心ダメージ" && s.type.get('jp') != "会心率" ? s.type.get('jp') + "パーセンテージ" : s.type.get('jp'),
