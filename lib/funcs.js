...
function addArt(chara) {
    ...
    chara.artifacts.forEach(r => {
        artData[artList[r.artifactData.equipType]] = {
            ...
            "main": {
                "option": !removeStatList.includes(r.mainstat.id) && r.mainstat.isPercent ? r.mainstat.type.get() + "パーセンテージ" : r.mainstat.type.get(),
        ...
        if (r.substats) {
            r.substats.total.forEach(s => {
                sub.push({
                    "option": !removeStatList.includes(s.id) && s.isPercent ? s.type.get() + "パーセンテージ" : s.type.get(),
