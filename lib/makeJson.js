...
module.exports = async function makeJson(uid, charaName, scoreType, option = {}) {
    try {
        let filePath;
        const cachePath = path.join(__dirname, "..", "./node_modules/enka-network-api/cache");

        if (option.length > 0) {
            if (Object.keys(option)[0] !== "cacheDirectory") throw new APIError(`${Object.keys(option)[0]} is not available folder path.`);
        };

        if (option.cacheDirectory) {
            if (typeof option.cacheDirectory !== "string") throw new APIError(`${option.cacheDirectory} is not a folder path`);
            filePath = path.join(__dirname, "..", option.cacheDirectory);
        } else {
            filePath = path.join(__dirname, "..", "./api-cache");
        };
